// `highball init` — scaffolds the .highball/ install unit and the Claude
// Code hooks. Deliberately conservative: it never overwrites an existing
// checks.yml, and it never edits an existing .claude/settings.json (hook
// merging is a human decision — it prints the snippet instead).
import { appendFileSync, existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { basename, join } from "node:path";
import YAML from "yaml";

const CHECKS_TEMPLATE = (project) => `# ${project}'s Highball rules — the file \`highball run\` reads and
# reports from. \`fast: true\` marks rules cheap enough to run on every
# agent edit; the rest join at turn end. \`todo: true\` declares a rule
# you're committed to but haven't built — tracked in run history,
# never a failure.
version: 1
project: ${project}

# Set \`enabled: false\` to switch this repo's checks off for everyone,
# without deleting rules or hooks. To switch them off only in YOUR
# checkout, leave this alone and \`touch .highball/disabled\` instead —
# it is gitignored, and takes effect on the next run.
#
# Anything below that differs on YOUR machine — the app runs in Docker
# here and on the host for the team, or a slower laptop needs a longer
# budget — goes in .highball/checks.local.yml (also gitignored), which
# may set \`exec\`, \`timeouts\` and \`reporting\`, and nothing else.

# Optional telemetry. The PostHog project key is write-only by design, so
# it is committed config — there is no login step and no credentials file.
# To keep it out of the repo instead, set HIGHBALL_POSTHOG_KEY in the
# environment (e.g. the env block of ~/.claude/settings.json) and leave
# this block out entirely.
# reporting:
#   posthog:
#     host: https://us.i.posthog.com
#     project_key: phc_your_key

# Wall-clock budgets, in seconds. A rule past its budget is killed along
# with everything it started, fails with "timed out", and is journaled
# that way. Defaults: fast 8, full 60, judge 300. Raise a kind here, or
# give one rule its own \`timeout:\` below.
# timeouts:
#   full: 120

# If this repo's toolchain lives in a container, declare the wrapper once
# and every rule runs through it; rules that belong on the host opt out
# with \`exec: host\`. Omit entirely for host-based setups.
# exec:
#   via: docker compose exec -T app

checks:
  # - id: unit-tests
  #   name: Unit tests
  #   run: npm test
  #   timeout: 300

  # - id: lint
  #   name: Lint & formatting
  #   run: npx eslint .
  #   fast: true
`;

// The hooks run the installed package's entry point directly. Highball is
// a dev dependency (that is step 0 of onboarding), and `npx` on top of a
// local install spends ~180ms per call resolving to the same file — paid
// on every agent tool call, most of which skip in a few milliseconds. A
// path into node_modules also can't be hijacked by the unscoped npm name
// `highball`, which belongs to an unrelated package.
const HOOK_COMMAND = "node node_modules/@profoundry-us/highball/bin/highball.js run";

// The fast hook matches Bash as well as the edit tools: agents in auto mode
// edit through Bash, and a hook on Write|Edit alone never fires for them.
// --if-changed keeps the Bash firings free when the tree hasn't moved.
// The Stop hook carries it too: a turn that edited nothing since the last
// full run (a question answered, a plan discussed) shouldn't pay for the
// whole suite. The stamps are per mode, so the fast hook's stamp never
// satisfies the Stop hook — an edit always gets its full run at turn end.
//
// A repo with no `fast: true` rule gets no fast hook at all: every firing
// would be node startup and a git status in exchange for nothing.
//
// The hook timeouts are backstops, not budgets: the runner times out each
// rule itself (`timeouts:` in checks.yml) and journals the result, so the
// hook's own limit should only ever fire if the runner is somehow wedged.
// Without one, Claude Code's default for a fast hook is ten minutes.
const FAST_HOOK = {
  matcher: "Write|Edit|Bash",
  hooks: [{ type: "command", command: `${HOOK_COMMAND} --fast --if-changed`, timeout: 120 }]
};
const STOP_HOOK = {
  hooks: [{ type: "command", command: `${HOOK_COMMAND} --if-changed`, timeout: 900 }]
};

export function hooksJson({ fast }) {
  return {
    hooks: {
      ...(fast ? { PostToolUse: [ FAST_HOOK ] } : {}),
      Stop: [ STOP_HOOK ]
    }
  };
}

// Whether an existing checks.yml has any rule worth a fast hook. Lenient
// by design: init must never refuse to scaffold over a config that
// `run` will later reject with a proper message, so anything unreadable
// counts as "unknown" and keeps the fast hook.
export function hasFastRules(checksPath) {
  try {
    const config = YAML.parse(readFileSync(checksPath, "utf8"));
    const checks = Array.isArray(config?.checks) ? config.checks : [];
    return checks.some((rule) => rule && rule.fast === true);
  } catch {
    return true;
  }
}

const GITIGNORE =
  "# A local, uncommitted `highball run` off switch.\ndisabled\n" +
  "# Per-checkout overrides of exec, timeouts and reporting.\nchecks.local.yml\n";

function ignores(ignorePath, name) {
  return readFileSync(ignorePath, "utf8").split("\n").some((line) => line.trim() === name);
}

export async function init() {
  const root = process.cwd();
  const project = basename(root);

  mkdirSync(join(root, ".highball"), { recursive: true });

  // A fresh template has no rules yet, so it can't say whether fast ones
  // are coming; it gets both hooks, and the onboarding guide tells the
  // agent to drop the fast hook if none materialise. An existing config
  // has already decided.
  const checksPath = join(root, ".highball", "checks.yml");
  let fast = true;
  if (existsSync(checksPath)) {
    console.log(`kept existing ${relative(checksPath, root)}`);
    fast = hasFastRules(checksPath);
    if (!fast) {
      console.log("no `fast: true` rules in it — scaffolding the Stop hook only " +
        "(a fast hook with nothing to run would still cost node startup on every tool call)");
    }
  } else {
    writeFileSync(checksPath, CHECKS_TEMPLATE(project));
    console.log(`created .highball/checks.yml (project: ${project}) — add your rules`);
  }
  const hooks = hooksJson({ fast });

  // Ignore the `disabled` marker from inside .highball/ rather than by
  // editing the repo's root .gitignore, which init doesn't own. It is
  // committed, so every clone inherits the protection and nobody has to
  // remember the setup step that keeps a local switch-off local.
  // The local overlay gets the same protection, and an install from before
  // it existed gets the line added: the failure it prevents is a per-machine
  // Docker wrapper landing in a commit and breaking every teammate's hooks.
  const ignorePath = join(root, ".highball", ".gitignore");
  if (!existsSync(ignorePath)) {
    writeFileSync(ignorePath, GITIGNORE);
    console.log("created .highball/.gitignore (keeps `disabled` and checks.local.yml out of commits)");
  } else if (!ignores(ignorePath, "checks.local.yml")) {
    appendFileSync(ignorePath, "# Per-checkout overrides of exec, timeouts and reporting.\nchecks.local.yml\n");
    console.log("added checks.local.yml to .highball/.gitignore");
  } else {
    console.log("kept existing .highball/.gitignore");
  }

  const settingsPath = join(root, ".claude", "settings.json");
  if (existsSync(settingsPath)) {
    console.log(
      "\n.claude/settings.json already exists — merge these hooks yourself:\n" +
        JSON.stringify(hooks, null, 2)
    );
  } else {
    mkdirSync(join(root, ".claude"), { recursive: true });
    writeFileSync(settingsPath, JSON.stringify(hooks, null, 2) + "\n");
    console.log(fast
      ? "created .claude/settings.json (fast checks on edit, full suite at turn end)"
      : "created .claude/settings.json (full suite at turn end)");
  }

  console.log(
    "\nnext: fill in .highball/checks.yml, and optionally uncomment the" +
      "\nreporting.posthog block to send runs to PostHog."
  );
  return 0;
}

function relative(path, root) {
  return path.startsWith(root) ? path.slice(root.length + 1) : path;
}
