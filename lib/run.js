...
export async function run(args) {
  ...
  if (args.includes("--if-changed") && fingerprint &&
      fingerprint === readStamp(config.project, process.cwd(), mode)) {
  ...
  if (fingerprint) {
    writeStamp(config.project, process.cwd(), "fast", fingerprint);
    if (!fastOnly && failures.length === 0) {
      writeStamp(config.project, process.cwd(), "full", fingerprint);
    }
  }
