...
export function stampPath(project, cwd, mode, dir = stampDir()) {
  const checkout = createHash("sha256").update(cwd).digest("hex").slice(0, 12);
  return join(dir, `${project}-${checkout}-${mode}`);
}
...
export function readStamp(project, cwd, mode, dir = stampDir()) {
  try {
    return readFileSync(stampPath(project, cwd, mode, dir), "utf8").trim();
  } catch {
    return null;
  }
}
...
export function writeStamp(project, cwd, mode, fingerprint, dir = stampDir()) {
  try {
    mkdirSync(dir, { recursive: true, mode: 0o700 });
    writeFileSync(stampPath(project, cwd, mode, dir), `${fingerprint}\n`);
  } catch {
    // ignore
  }
}
